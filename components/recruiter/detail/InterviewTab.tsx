        ...
        <CardContent>
          <div className="flex flex-col gap-4">
            {[
              {
                speaker: "AI",
                text: "Can you explain the difference between React's reconciliation algorithm and the fiber architecture? Specifically, how does fiber enable concurrent rendering?",
              },
              {
                speaker: candidate.name,
                text: "Sure. React's reconciliation algorithm is the process by which React updates the virtual DOM. Fiber is the re-implementation of React's core algorithm that was introduced in React 16. The key difference is that Fiber breaks rendering work into small units called fibers — essentially linked list nodes — and allows React to pause, abort, or resume work. This enables concurrent features like useTransition and Suspense.",
              },
              {
                speaker: "AI",
                text: "That's precise. Can you walk me through what happens when setState is called inside a React component? Trace the lifecycle from update request to DOM commit.",
              },
              {
                speaker: candidate.name,
              ...
              <div
                key={i}
                className={`flex gap-3 ${line.speaker !== "AI" ? "justify-end" : ""}`}
              >
                {line.speaker === "AI" && (
                  <div className="w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-primary text-xs font-mono shrink-0">
                    AI
                  </div>
                )}
                <div
                  className={`max-w-2xl rounded-xl px-4 py-3 text-sm leading-relaxed ${line.speaker === "AI" ? "bg-secondary" : "bg-primary text-primary-foreground"}`}
                >
                  {line.text}
                </div>
              </div>
