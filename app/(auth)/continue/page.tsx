...
import { authFormSchema } from "@/lib/form-validator";
...
function SignupForm() {
  ...
  const onEmailSubmit = methods.handleSubmit(async (formData: AuthFormTypes) => {
    console.log("onEmailSubmit invoked", formData);
    setLoading(true);
    const email = formData.emailVerify.email;
    try {
  ...
  const onOtpSubmit = methods.handleSubmit(async (formData: AuthFormTypes) => {
    console.log("onOtpSubmit invoked", formData);
    setLoading(true);

    const email = formData.emailVerify.email ?? emailParam;
    const otp = formData.otpVerify.otp;
