...
function SignupForm() {
  ...
  const handleOtpSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setLoading(false)
      router.push(role === 'candidate' ? '/candidate/profile/build' : '/recruiter/search')
    }, 1000)
  }
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      <div className="w-full max-w-md">
        ...
        {step === 'role' && (
          <div>
            <h1 className="font-display text-2xl font-semibold text-center mb-2">Create your account</h1>
            <p className="text-muted-foreground text-sm text-center mb-8">Who are you joining as?</p>
            <div className="grid grid-cols-2 gap-4 mb-6">
              {(['candidate', 'recruiter'] as Role[]).map(r => (
                ...
                >
                  <div className={cn('w-12 h-12 rounded-full flex items-center justify-center', role === r ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground')}>
                    {r === 'candidate' ? <User size={22} /> : <Briefcase size={22} />}
                  </div>
                  <div className="text-center">
                    <div className="font-medium capitalize">{r}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">
                      {r === 'candidate' ? 'Take an AI interview' : 'Hire verified talent'}
                    </div>
                  </div>
                </button>
              ))}
        ...
        {step === 'email' && (
          <div>
            ...
            <p className="text-muted-foreground text-sm text-center mb-8">We&apos;ll send a one-time code to verify your email.</p>
        ...
        {step === 'otp' && (
          <div>
            <h1 className="font-display text-2xl font-semibold text-center mb-2">Check your inbox</h1>
