    ...
    <>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        {[
          ...
          <Card
            key={p.name}
            className={p.name === "Pro" ? "border-primary" : ""}
          >
            <CardContent className="py-5">
              <div className="flex items-center justify-between mb-2">
                <span
                  className={`text-xs font-mono px-2 py-0.5 rounded-full ${planColors[p.name]}`}
                >
                  {p.name}
                </span>
                {p.name === "Pro" && (
              ...
              <div className="font-display text-xl font-semibold mb-0.5">
                {p.price}
              </div>
              ...
              {plans.map((p) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs font-mono px-2 py-0.5 rounded-full ${planColors[p.plan]}`}
                    >
                      {p.plan}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 hidden md:table-cell font-mono text-sm">
                    {p.monthly}
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell text-muted-foreground">
                    {formatDate(p.renewsOn)}
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={
