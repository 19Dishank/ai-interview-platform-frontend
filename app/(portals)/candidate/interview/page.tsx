...
const transcript = [
  {
    speaker: "AI",
    text: "Hello Arjun, I'm your Verquo interviewer for today's Frontend Engineering assessment. We'll be covering React internals, performance optimisation, and system design. I'll also ask you a live coding challenge toward the end. Are you ready to begin?",
  },
  { speaker: "You", text: "Yes, I'm ready. Thanks." },
  {
    speaker: "AI",
    text: "Great. Let's start with a foundational question. Can you explain the difference between React's reconciliation algorithm and the fiber architecture it uses under the hood? Specifically, how does fiber enable concurrent rendering?",
  },
  {
    speaker: "You",
    text: "Sure. React's reconciliation algorithm is the process by which React updates the virtual DOM. Fiber is the re-implementation of React's core algorithm that was introduced in React 16. The key difference is that Fiber breaks rendering work into small units called fibers — essentially linked list nodes representing components — and allows React to pause, abort, or resume work. This is what enables concurrent features like useTransition and Suspense. Before Fiber, the stack-based reconciler was synchronous and couldn't be interrupted once started.",
  },
  {
    speaker: "AI",
    text: "That's a precise answer. Can you walk me through what happens when setState is called inside a React component? Trace the lifecycle from state update request to DOM commit.",
  },
];
        ...
        <div className="flex-1 p-6 overflow-y-auto max-h-[calc(100vh-56px)]">
          ...
          <div className="flex flex-col gap-5">
            {transcript.slice(0, visibleLines).map((line, i) => (
              <div
                key={i}
                className={`flex gap-3 ${line.speaker === "You" ? "justify-end" : "justify-start"}`}
              >
                {line.speaker === "AI" && (
                  <div className="w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-primary text-xs font-mono shrink-0 mt-0.5">
                    AI
                  </div>
                )}
                <div
                  className={`max-w-lg rounded-xl px-4 py-3 text-sm leading-relaxed ${line.speaker === "AI" ? "bg-secondary text-foreground" : "bg-primary text-primary-foreground"}`}
                >
                  {line.text}
                </div>
                {line.speaker === "You" && (
                  <div className="w-7 h-7 rounded-full bg-primary flex items-center justify-center text-primary-foreground text-xs font-semibold shrink-0 mt-0.5">
                    A
                  </div>
                )}
              </div>
            ...
            {visibleLines < transcript.length && (
              <div className="flex gap-2 items-center">
                <div className="w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-primary text-xs font-mono">
                  AI
                </div>
